...
namespace FMODUnityTools
{
    ...
    public class SpatialAudioManager : MonoBehaviour
    {
        ...
        public void RegisterRoomAwareInstance(EventInstance eventInstance, SpatialAudioRoom fixedRoom = null)
        {
            ...
            CheckRegisteredInstanceValidity();//JULIAN ADDED THIS
        ...
        SpatialAudioRoom lastRoom = null;
        ...
        void LateUpdate()
        {
            ...
            if (activeModes.HasFlag(ActiveModes.PropagationCost) && currentListenerRooms != null && currentListenerRooms.Count > 0)
            {
                ...
                if (lastRoom != currentListenerRoom)
                {
                    roomString = currentListenerRoom.ToString();
                }

                lastRoom = currentListenerRoom;

                //if (newRoomString != roomString)
                //{
                //    print("current listener room: " + currentListenerRoom);
                //}
        ...
        private void CheckRegisteredInstanceValidity()
        {
            for (int i = registeredInstances.Count - 1; i > -1; i--)
            {
                ...
                if (playbackState == PLAYBACK_STATE.STOPPED || playbackState == PLAYBACK_STATE.STOPPING)//JULIAN ADDED THIS
                {
                    registeredInstances.RemoveAt(i);
                }
            }
        ...
        private void CalculatePropagationCosts(List<RoomAwareInstance> instances)
        {
            foreach (var instance in instances)
            {
                ...
                for (int i = 0; i < roomPair.routeAlternatives.Count; i++)
                {
                    ...
                    // Debug.Log($"Processing routeAlternative {i}: {string.Join(", ", routeAlternative)}");
                    // Debug.Log($"Route nodes for routeAlternative {i}: {string.Join(", ", routeAlternative.Select(ra => ra.gameObject.name + " (" + ra.transform.position.ToString() + ")"))}");
        ...
        private void ScaleRolloffDistance(float directRouteLength, float routeLength, RoomAwareInstance instance)
        {
            ...
            if (instance.eventInstance.isValid())
            {
                ...
                instance.eventInstance.getDescription(out EventDescription ed);
                ed.getPath(out string p);
